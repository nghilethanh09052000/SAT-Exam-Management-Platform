...
export function ExerciseCard({
  ...
  const diffLabel = t(difficulty as 'easy' | 'medium' | 'hard')
    ...
    >
      ...
      <div className="mt-4 flex items-center gap-3 text-xs font-bold text-[#9aa2b6]">
        <span className="flex items-center gap-1">
          ...
          {t('questionCount', { n: questionCount })}
        </span>
        <span className="flex items-center gap-1">
          <svg className="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
            <path strokeLinecap="round" strokeLinejoin="round" d="M12 6v6l4 2m6-2a10 10 0 11-20 0 10 10 0 0120 0z" />
          </svg>
          {t('minutesShort', { n: estimatedMinutes })}
        </span>
      </div>
      ...
      {pct !== null && (
        <div className="mt-4">
          <div className="mb-1 flex items-center justify-between text-xs font-black">
            <span className="text-[#6a7286]">{t('bestScore')}</span>
