-- Migration: 00043_exam_autosave_rpcs.sql
-- Add narrow RPCs for the hottest in-test write paths. These avoid verbose
-- PostgREST-generated update/upsert SQL while preserving ownership and
-- in-progress status checks inside the database.

CREATE OR REPLACE FUNCTION public.save_submission_progress(
...
BEGIN
  ...
  SET
    ...
    current_module = p_current_module,
    updated_at = now()
  WHERE s.id = p_submission_id
    AND s.student_id = v_student_id
    AND s.status = 'in_progress'::public.submission_status
    AND (
      s.current_question_id IS DISTINCT FROM p_current_question_id
      OR s.current_module IS DISTINCT FROM p_current_module
    )
  RETURNING
    s.id,
...
REVOKE ALL ON FUNCTION public.save_submission_progress(UUID, UUID, TEXT) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.upsert_submission_answer(UUID, UUID, UUID, TEXT, BOOLEAN, JSONB, TEXT, JSONB, INTEGER) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION public.save_submission_progress(UUID, UUID, TEXT) TO authenticated;
