...
--
-- Fix: convert to FOR EACH STATEMENT with a transition table (REFERENCING NEW
-- TABLE AS new_rows). Postgres passes all affected rows to one trigger
-- invocation. The new function does a single JOIN + bulk INSERT regardless of
-- how many wrong answers exist — O(1) queries instead of O(n).
--
...
BEGIN
  ...
  -- ON CONFLICT DO NOTHING is idempotent thanks to the unique constraint
  -- added in 00020_error_log_unique_entries (submission_id, question_id).
...
-- Drop the old per-row trigger, create the new per-statement trigger.
-- REFERENCING NEW TABLE AS new_rows works for both INSERT and UPDATE events.
