-- Migration: 00027_treat_max_retakes_as_total_attempts.sql
-- The UI label is "Số lần làm bài tối đa", so assignment_instances.max_retakes
-- now represents the maximum total attempts, not extra retakes.
...
BEGIN
  ...
  v_max_attempts := GREATEST(1, COALESCE(v_instance.max_retakes, 1));
